...
#ifdef __ARM_NEON__
  if(!no_simd)
    return align2D_NEON(cur_img, ref_patch_with_border, ref_patch, n_iter, cur_px_estimate, tau_scale_);
#endif

  /*   the tracking reslut is poor than no sse2
  #ifdef __SSE2__
  return align2D_SSE2(cur_img,ref_patch_with_border,ref_patch,n_iter,cur_px_estimate, tau_scale_);
  #endif
  */
  const int halfpatch_size_ = 4;
...
//         << "\t new chi2 = " << new_chi2 << endl;
  ...
  tau_scale_ = sqrt(update[0]*update[0]+update[1]*update[1]);
    ...
    double& tau_scale_)
{
  ...
  tau_scale_ = sqrt(update_u*update_u+update_v*update_v);
  cur_px_estimate << u, v;
  return converged;
}
