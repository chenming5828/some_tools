...
{
  bool found = false;
          //SVO_DEBUG_STREAM("8888888888888");
...
// void MapPointCandidates::newCandidatePoints(Point* points[], size_t size)
// void MapPointCandidates::newCandidatePoints(std::vector<Point*>& points, size_t size)
// {
//   boost::unique_lock<boost::mutex> lock(mut_);
//   for (size_t i = 0; i < size; i++) {
//     points[i]->type_ = Point::TYPE_CANDIDATE;
//     candidates_.push_back(PointCandidate(points[i], points[i]->obs_.front()));  
//   }
// }
...
// void MapPointCandidates::deleteCandidatePoints(std::vector<Point*>& points)
// {
//   boost::unique_lock<boost::mutex> lock(mut_);
//   for (auto& pit : points) {
//     for(auto it = candidates_.begin(), ite = candidates_.end(); it != ite; ++it)
