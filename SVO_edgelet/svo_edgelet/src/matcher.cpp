    ...
    Vector2d& px_cur)
{
  ...
  // std::cout << "!!!!!!affine cost : " << reprojCellLoopduration.count() << " us." << std::endl;

  search_level_ = warp::getBestSearchLevel(A_cur_ref_, Config::nPyrLevels()-1);

  auto reprojCellLoopStart2 = std::chrono::steady_clock::now();  
  ...
  auto reprojCellLoopEnd2 = std::chrono::steady_clock::now();
  auto reprojCellLoopduration2 = std::chrono::duration_cast<std::chrono::microseconds>(reprojCellLoopEnd2 - reprojCellLoopStart2);
  // std::cout << "!!!!!!affine2 cost : " << reprojCellLoopduration2.count() << " us." << std::endl;
  ...
  else
  {
    success = feature_alignment::align2D(
      ...
      options_.align_max_iter, px_scaled, tau_scale);
  }
  auto alignEnd = std::chrono::steady_clock::now();  
  auto alignduration = std::chrono::duration_cast<std::chrono::microseconds>(alignEnd - alignStart);
  // std::cout << "!!!!!!aligne cost : " << alignduration.count() << " us." << std::endl;
  ...
  if(epi_length_ < 2.0)
  {
    ...
    if(options_.align_1d)
      ...
      res = feature_alignment::align2D(
          cur_frame.img_pyr_[search_level_], patch_with_border_, patch_,
          options_.align_max_iter, px_scaled, tau_scale_);
    if(res)
    {
      px_cur_ = px_scaled*(1<<search_level_);
      static const double random_err = 0.02;
      tau_scale_ *= (1<<search_level_);
      tau_scale_ += random_err;
  ...
  if(zmssd_best < PatchScore::threshold())
  {
    if(options_.subpix_refinement)
    {
      ...
      if(options_.align_1d)
        ...
        res = feature_alignment::align2D(
            cur_frame.img_pyr_[search_level_], patch_with_border_, patch_,
            options_.align_max_iter, px_scaled, tmp_tau_scale);
      if(res)
      {
        px_cur_ = px_scaled*(1<<search_level_);
        if(depthFromTriangulation(T_cur_ref, ref_ftr.f, cur_frame.cam_->cam2world(px_cur_), depth))
