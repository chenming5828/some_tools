    ...
    std::vector< std::pair<FramePtr,std::size_t> >& overlap_kfs)
{
  ...
  // std::cout << "totally feature points count : " << cc << std::endl;
  // std::cout << "close kf count : " << close_kfs.size() << std::endl;
  ...
  // std::cout << "!!!!!!repj candidate cost : " << reprojduration.count() << " ms." << std::endl;
  ...
  auto reprojCellStart = std::chrono::steady_clock::now();
  int ccc = 0;
  for(size_t i = 0; i < grid_.cells.size(); ++i)
  {
  ...
  // std::cout << "repj cell count : " << ccc << std::endl;
  auto reprojCellEnd = std::chrono::steady_clock::now();  
  auto reprojCellduration = std::chrono::duration_cast<std::chrono::milliseconds>(reprojCellEnd - reprojCellStart);
  // std::cout << "!!!!!!repj cells cost : " << reprojCellduration.count() << " ms." << std::endl;
  // std::cout << "grid cells count : " << grid_.cells.size() << std::endl;
...
inline bool Reprojector::reprojectCell(Cell& cell, FramePtr frame)
{
  ...
  while(it != cell.end())
  {
    ...
    std::vector<Point*> FutureDeleteCandPoints;
    FutureDeleteCandPoints.clear();
    auto reprojCellLoopStart = std::chrono::steady_clock::now();  
  
    auto tmp_px = it->px;    
    if(options_.find_match_direct)
      found_match = matcher_.findMatchDirect(*it->pt, *frame, it->px);

    auto reprojCellLoopEnd = std::chrono::steady_clock::now();  
    auto reprojCellLoopduration = std::chrono::duration_cast<std::chrono::microseconds>(reprojCellLoopEnd - reprojCellLoopStart);
    // std::cout << "!!!!!!repj cells loop cost : " << reprojCellLoopduration.count() << " us." << std::endl;
    ...
    if(!found_match)
    {
      ...
      if(it->pt->type_ == Point::TYPE_CANDIDATE  && it->pt->n_failed_reproj_ > 30)
        ...
        // FutureDeleteCandPoints.push_back(it->pt);
    ...
    // map_.point_candidates_.deleteCandidatePoints(FutureDeleteCandPoints);

    it->pt->n_succeeded_reproj_++;
    // Give more trust to depth filter.
...
inline bool Reprojector::reprojectPoint(FramePtr frame, Point* point)
{
