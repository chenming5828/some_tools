...
SparseAlign::SparseAlign(int max_level, int min_level, int n_iter, bool display, bool verbose) :
        display_(display),
...
Matrix<double, 6, 6> SparseAlign::getFisherInformation()
{
    ...
    for(int y=0; y<patch_size_; ++y)
    {
      ...
      for(int x=0; x<patch_size_; ++x, ++ref_img_ptr, ++cache_ptr, ++pixel_counter)
      {
        ...
        *cache_ptr = w_ref_tl*ref_img_ptr[0] + w_ref_tr*ref_img_ptr[1] + w_ref_bl*ref_img_ptr[stride] + w_ref_br*ref_img_ptr[stride+1];
        ...
        // std::cout <<"-----" <<jacobian_cache_.rows() <<" : "<<  jacobian_cache_.cols() <<std::endl;
      }
    }
  }
...
void SparseAlign::finishIteration()
{
  if(display_)
  {
