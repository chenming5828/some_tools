...
command_f=""
echo "check EMMC condition..."
if [ -d /media/internal ];then
	echo "find upgrade package..."	
	if [ -f /media/internal/upgrade_package.tar ];then
		echo "Find upgrade_package.tar in EMMC partition..."
	else
		echo "Can't Find upgrade_package.tar in EMMC partition..."
		exit 1
	fi
else
	echo "can't find EMMC partition Directory,plz check your EMMC partition..."
	exit 1
fi
...
/bin/tar -xvf /media/internal/upgrade_package.tar -C /tmp/
wait
