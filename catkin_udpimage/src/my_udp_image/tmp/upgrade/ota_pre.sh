...
command_f=""
echo "check /tmp condition..."
if [ -d /tmp ];then
	echo "find upgrade package......"	
	if [ -f /tmp/upgrade_package.tar ];then
		echo "Find upgrade_package.tar in /tmp..."
	else
		echo "Can't Find upgrade_package.tar in /tmp..."
		exit 1
	fi
else
	echo "can't find /tmp Directory,plz check your ota upgrede path..."
	exit 1
fi
...
/bin/tar -xvf /tmp/upgrade_package.tar -C /tmp/
wait
